...
import { closeBrowser, decrementActiveConversions, generateOutput } from './generate-output';
import { getHtml } from './get-html';
import { getOutputFilePath } from './get-output-file-path';
import { getMarginObject } from './helpers';
import { readFile } from './read-file';
import { closeServer } from './serve-dir';
...
export const convertMdToPdf = async (
	...
	config: Config,
	{
		...
		args?: CliArgs;
		/** By default, auto-close the browser instance after conversion is done. Set to `false` to stop this behavior (and don't forget to manually call `await closeBrowser()`). */
		autoCloseBrowser?: boolean;
		/** If passed, closes the given server after conversion is done. If you don't pass this, don't forget to manually call `await closeServer(server)`. */
		server?: Server;
	},
) => {
	try {
